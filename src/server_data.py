...
class ServerData:
    ...
    def __init__(self, categories, server_cache_dir, basedir_statistics_records, mail_handler=None):
        ...
        self.audio_paths = sorted([os.path.join(self.server_cache_dir, fn) for fn in os.listdir(self.server_cache_dir) if
                                   not fn.startswith('.') and fn.endswith('.json')])

        open(os.path.join(basedir_statistics_records, 'sound_tracks.csv'), 'w').write(
            "\n".join(['sound_index,sound_name', *[f'{i},{ap}' for i, ap in enumerate(self.audio_paths)]]))
        self.users_filepath = os.path.join(basedir_statistics_records, 'users.csv')
        ...
        self.metadata_from_users_filepath = os.path.join(basedir_statistics_records, 'metadata_from_users.csv')
    ...
    def audio_track_metadata(self, user):
        ...
        try:
            ...
            return dict(
                sound_index=index,
                sound_name=os.path.basename(audio_path).split('.')[0],
